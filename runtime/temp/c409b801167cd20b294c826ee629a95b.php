<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:65:"D:\xampp\htdocs\TMES\public/../app/home\view\Tpcusinfo\index.html";i:1472032171;}*/ ?>
...
<div id="__MODULE__PagePanel" class="easyui-layout" data-options="fit : true,border : false">
	<div id="__MODULE__SearchTab" title="<?php echo \think\Lang::get('LANG_SEARCH'); ?>" data-options="region:'north',closedTitle:'<?php echo \think\Lang::get('LANG_SEARCH'); ?>',border:true,collapsed:false,iconCls:'icon-search'" style="height: 130px;overflow: auto;" align="center">
		<form id="__MODULE__SearchForm" onsubmit="$('#__MODULE__Search').click();return false;">
			<table class="tableForm">
                ...
                <input id="searchC" class="easyui-combobox"  name="searchC" data-options=" valueField: 'id',
                                                                                               textField: 'text',
                                                                                               url: '/home/tpcusinfo/x_xgetsearchC',
                                                                                               panelHeight:'auto'" />
                </th>
                <td>
				...
				<tr>
					<td style="width: 170px;"><?php echo \think\Lang::get('LANG_KEYWORDS'); ?></td>
					<td><input name="searchKeyWords" style="width: 315px;" /></td>
				</tr>
			</table>
            <div>
            	<a id="__MODULE__Search" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-search'" onclick="__MODULE__SearchFun();"><?php echo \think\Lang::get('LANG_SEARCH'); ?></a>
                <a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-cross'" onclick="__MODULE__CleanFun();"><?php echo \think\Lang::get('LANG_RESET'); ?></a>
            </div>
		</form>
	</div>
	<div data-options="region:'center',border:false"> 
		<table id="__MODULE__Datagrid" class="easyui-datagrid"
		 data-options="url : '/home/tpcusinfo/x_xgetTableJson',
			fit : true,
			border : false,//定义边框
...
<script type="text/javascript"> 
	function formatStatus(value, row, index) { //console.info(row);
		if (value==1){
			return '<font color=green><?php echo \think\Lang::get('LANG_CUS_AVAIL'); ?></font>';
		}else if(value==-1){
			return '<font color=red><?php echo \think\Lang::get('LANG_CUS_DEL'); ?></font>';
		}else {
			return '<font color=red><?php echo \think\Lang::get('LANG_CUS_FORBID'); ?></font>';
		}				
	}
	...
	function __MODULE__EditFun() {
		...
		$('<div/>').dialog({
			href : '/home/tpcusinfo/x_xshowEdit/id/'+rows[0].id,
			width : 630,
			height : 400,
            maximized:true,
			modal : true,
    ...
    function __MODULE__exportExcel() { 
    	$('#__MODULE__SearchForm').form('submit', {
    		url : '/home/tpcusinfo/exportExcel',
            success : function(result) {
				try {
					return true;
				} catch (e) {
    ...
    function __MODULE__viewFun() {
		...
		if (rows.length > 1) {
			...
			$('<div/>').dialog({
				href : '/home/tpcusinfo/x_xshow/id/'+rows[0].id,
				width : 630,
				height : 400,
				maximized:true,
				modal : true,
