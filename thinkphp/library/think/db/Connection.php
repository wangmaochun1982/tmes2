...
abstract class Connection
{
     ...
     */
     public function execProcedure($pName,$pValue) {
     	...
     	$pValue = array_change_key_case($pValue, CASE_UPPER);//强制下标为小写
        $this->initConnect();
        if (!$this->linkID) {
            return false;
        }
        ...
        // 释放前次的查询结果
		 ...
		 foreach ($argRow as $key=>$val) {
            if($val['in_out'] == 'OUT'){
            	$this->PDOStatement->bindParam($val['argument_name'], $pValue[$val['argument_name']], PDO::PARAM_STR|PDO::PARAM_INPUT_OUTPUT, 4000);
            }else{
            	$this->PDOStatement->bindParam($val['argument_name'], $pValue[$val['argument_name']]);
            }
         }
         $this->PDOStatement->execute();
         return $pValue;
